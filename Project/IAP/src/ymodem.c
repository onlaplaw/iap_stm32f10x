...
uint32_t FlashDestination = APPLICATION_ADDRESS; /* Flash user program offset */
  ...
  *length = 0;
  ...
  switch (c)
  {
    ...
    case STX:
      ...
      break;
    case EOT:
      for(EraseCounter = 0; (EraseCounter < 1) && (MY_FLASHStatus == FLASH_COMPLETE); EraseCounter++)
      {
          MY_FLASHStatus = FLASH_ErasePage(0x08002800);
      }
      FLASH_ProgramWord(0x08002800, 0);
